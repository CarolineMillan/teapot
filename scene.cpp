...
void Scene::trace(Ray ray, Object *objects, Hit &hit) //does the ray intersect with an object in the scene?
{
	Hit current_hit;
	...
	if (best_hit.flag)
	{
		...
		while (light != (Light *)0)
		{
			...
			if (lit)
			{
				...
				light->get_intensity(best_hit.position, scaling); //write get_intensity?

				best_hit.what->material->compute_light_colour(viewer, best_hit.normal, ldir, intensity); //write this function too?
